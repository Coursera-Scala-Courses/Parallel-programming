...
package object barneshut {
  ...
  case class Fork(
    ...
    def insert(b: Body): Fork = {
      if(b.x<nw.centerX && b.y<nw.centerY)
        Fork(nw.insert(b),ne,sw,se)
      else if(b.x<nw.centerX && b.y>=nw.centerY)
  ...
  extends Quad {
    ...
    val mass = bodies.foldLeft(0.0f)((a,b)=>a+b.mass)
    val massX = bodies.foldLeft(0.0f)((a,b)=>a+(b.mass*b.x))/mass
    val massY = bodies.foldLeft(0.0f)((a,b)=>a+(b.mass*b.y))/mass
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      val x: Int = ((b.x-boundaries.minX)/sectorSize).toInt max 0 min sectorPrecision-1
      val y: Int = ((b.y-boundaries.minY)/sectorSize).toInt max 0 min sectorPrecision-1
      this(x,y)+=b
      this
    }
